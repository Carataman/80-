...
function doRemCamera() {
    if (Object.keys(pushConfigs).length) {
        return runAlertDialog('Сохраните настройки!');
    }
    ...
    runConfirmDialog('Удалить камеру ' + deviceName + '?', function () {
...
function doDeleteFile(path, callback) {
    ...
    runConfirmDialog('Хотите удалить данные?', function () {
        showModalDialog('<div class="modal-progress"></div>', null, null, true);
    ...
    function fetchCameraList() {
        ...
        ajax('GET', basePath + 'config/list/', null, function (data) {
            ...
            if (isAdmin()) {
                ...
                if (!query.camera_ids) {
                    cameraSelect.append('<option value="add">Подключить</option>');
                }
...
function runLoginDialog(retry) {
    ...
    var form =
            $('<form action="' + basePath + 'login/" target="temp" method="POST"><table class="login-dialog">' +
                ...
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Логин</span></td>' +
                    '<td class="dialog-item-value"><input type="text" name="username" class="styled" id="usernameEntry" autofocus></td>' +
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Пароль</span></td>' +
                ...
                '</tr>' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Запомнить</span></td>' +
    ...
    var params = {
        ...
        content: form,
        buttons: [
            {caption: 'Отмена', isCancel: true, click: function () {
                tempFrame.remove();
            }},
            {caption: 'Войти', isDefault: true, click: function () {
...
function runAddCameraDialog() {
    ...
    var content =
            $('<table class="add-camera-dialog">' +
                '<tr>' +
                    '<td class="dialog-item-label"><span class="dialog-item-label"></span></td>' +
                    '<td class="dialog-item-value"><select class="styled" id="typeSelect">' +
//                        (hasLocalCamSupport ? '<optionvalue="v4l2">Local V4L2 Camera</option>' : '') +
//                        (hasLocalCamSupport ? '<option value="mmal">Local MMAL Camera</option>' : '') +
                          (hasNetCamSupport ? '<option value="netcam">Сетевая камера</option>' : '') +
...
//                        '<option value="mjpeg">Simple MJPEG Camera</option>' +
                ...
                '<tr class="motioneye netcam mjpeg">' +
                    ...
                    '<td class="dialog-item-value"><input type="text" class="styled" id="urlEntry" placeholder="rtsp://ip-address:8554/stream"></td>' +
                    '<td><span class="help-mark" title="URL камеры (пример: rtsp://ip-address:8554/stream)">?</span></td>' +
                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Имя пользователя</span></td>' +
                    '<td class="dialog-item-value"><input type="text" class="styled" id="usernameEntry" placeholder="имя пользователя..."></td>' +

                '</tr>' +
                '<tr class="motioneye netcam mjpeg">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Пароль</span></td>' +
                    '<td class="dialog-item-value"><input type="password" class="styled" id="passwordEntry" placeholder="пароль..."></td>' +

                '</tr>' +
                '<tr class="v4l2 motioneye netcam mjpeg mmal">' +
                    '<td class="dialog-item-label"><span class="dialog-item-label">Камера</span></td>' +
...
function runMediaDialog(cameraId, mediaType) {
        ...
        function addEntries() {
            ...
            entries.forEach(function (entry) {
                ...
                if (!entryDiv) {
                    ...
                    var downloadButton = $('<div class="media-list-download-button button">Загрузить</div>');
                    entryDiv.append(downloadButton);

                    var deleteButton = $('<div class="media-list-delete-button button">Удалить</div>');
                    if (isAdmin()) {
    ...
    if (isAdmin()) {
        var deleteAllButton = $('<div class="media-dialog-button media-dialog-delete-all-button">Удалить все</div>');
    ...
    ajax('GET', basePath + mediaType + '/' + cameraId + '/list/', null, function (data) {
        ...
        if ($(window).width() < 1000) {
            ...
            title = 'Изображения с камеры ' + data.cameraName;
        }
        else {
            title = 'Запись с камеры ' + data.cameraName;
        }
...
//                            '<div class="button icon camera-top-button mouse-effect media-pictures" title="open pictures browser"></div>' +
                            '<div class="button icon camera-top-button mouse-effect media-movies" title="Открыть журнал видео"></div>' +
    ...
    function updateCameras(cameras) {
        ...
        if ($('#cameraSelect').find('option').length < 2 && isAdmin() && !query.camera_ids) {
            ...
            var addCameraLink = $('<div class="add-camera-message">' +
                    '<a href="javascript:runAddCameraDialog()">Нет подключенной камеры, нажмите для добавления...</a></div>');
